...
#Import Data
df=loadRData("RawData/ExpertSample2025_rawAnonymData.RData")
...
df$totalMissing=(df$Task1Choice=="")+(df$Task2Choice=="")+(df$Task3Choice=="")+(df$Task4Choice=="")+(df$Task5Choice=="")+
  (df$Task6Choice=="")+(df$Task7Choice=="")+(df$Task8Choice=="")+(df$Task9Choice=="")+(df$Task10Choice=="")
table(df$totalMissing)
#Keep only those with all data
df=df[df$totalMissing==0,]
...
#longdf=longdf[!is.na(longdf$state),] #added for expert sample
...
save(longdf,file="TreatedData/ExpertSample2025.RData")
